...
must_haves:
  truths:
    - "Web host loads config/cookies.json as a fallback config source (before AddSqlite), so existing cookies.json entries are preserved; SQLite bili_appsettings remains the highest-priority cookie config source"
